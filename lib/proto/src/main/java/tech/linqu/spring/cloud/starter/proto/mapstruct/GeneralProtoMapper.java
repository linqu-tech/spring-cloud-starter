...
package tech.linqu.spring.cloud.starter.proto.mapstruct;

import lombok.RequiredArgsConstructor;
import org.springframework.boot.autoconfigure.data.web.SpringDataWebProperties;
import org.springframework.data.domain.Page;
import org.springframework.data.domain.Pageable;
import tech.linqu.spring.cloud.starter.proto.imports.common.PageablePb;
import tech.linqu.spring.cloud.starter.proto.imports.common.PagingPb;
...
public class GeneralProtoMapper {
    ...
    public PagingPb pagingPb(Page<?> page) {
        if (page.getPageable().isPaged()) {
            return new PagingPb()
                .setPage(dataWebProperties.getPageable().isOneIndexedParameters()
                    ? page.getNumber() + 1 : page.getNumber()
                )
